( function( $ ) {
	...
	wpea.getId = function( form ) {
		return parseInt( $( 'input[name="_wpea"]', form ).val(), 10 );
	};
	...
	wpea.submit = function( form ) {
		...
		wpea.triggerEvent( $form.closest( 'div.wpea' ), 'beforesubmit', detail );
		...
		$.ajax( {
			...
			url: wpea.apiSettings.getRoute(
				'/forms/' + wpea.getId( $form ) + '/feedback' ),
			data: formData,
			dataType: 'json',
			processData: false,
			contentType: false
