...
class FormTag implements ArrayAccess {
	...
	public function get_id_option() {
		...
		if (
			! $option or
			str_starts_with( $option, 'wpea' ) or
	...
	public function get_date_option( $option_name ) {
		...
		$date = apply_filters( 'wpea_form_tag_date_option',
			null,
	...
	public function get_default_option( $default_value = '', $args = '' ) {
		...
		foreach ( $options as $opt ) {
			...
			if ( 'user_' == substr( $opt, 0, 5 ) and is_user_logged_in() ) {
				...
				$vals = array_map( 'wpea_sanitize_query_var', $vals );
				...
				if ( $contact_form = EmailArts::get_current() ) {
	...
	public function get_data_option( $args = '' ) {
		...
		return apply_filters( 'wpea_form_tag_data_option', null, $options, $args );
	}
