...
class Validation implements ArrayAccess {
	 ...
	 *
	 * @param WPEA_FormTag|array|string $context Context representing the
	 *                                   target field.
	 * @param WP_Error|string $error The error of the field.
	 */
	public function invalidate( $context, $error ) {
		if ( $context instanceof FormTag ) {
			$tag = $context;
		} elseif ( is_array( $context ) ) {
			$tag = new FormTag( $context );
		} elseif ( is_string( $context ) ) {
			$tags = wpea_scan_form_tags( array( 'name' => trim( $context ) ) );
			$tag = $tags ? new FormTag( $tags[0] ) : null;
		}

		$name = ! empty( $tag ) ? $tag->name : null;

		if ( empty( $name )
		or ! wpea_is_name( $name ) ) {
			return;
		}
