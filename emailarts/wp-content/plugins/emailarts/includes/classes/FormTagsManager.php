...
function wpea_scan_form_tags( $cond = null ) {
    $form = EmailArts::get_current();

    if ( $form ) {
        return $form->scan_form_tags( $cond );
    }
...
/**
 * Wrapper function of FormTagsManager::add().
 */
function wpea_add_form_tag($tag_types, $callback, $features = '')
{
...
/**
 * Wrapper function of FormTagsManager::remove().
 */
function wpea_remove_form_tag($tag_type)
{
    $manager = FormTagsManager::get_instance();
...
/**
 * Wrapper function of FormTagsManager::replace_all().
 */
function wpea_replace_all_form_tags($content)
{
    $manager = FormTagsManager::get_instance();
...
/**
 * Wrapper function of FormTagsManager::tag_type_supports().
 */
function wpea_form_tag_supports($tag_type, $feature)
{
    $manager = FormTagsManager::get_instance();
...
class FormTagsManager
{
     ...
     *
     * @return FormTagsManager The singleton manager.
     */
     ...
     *
     * @return array Array of FormTag objects.
     */
    ...
    public function replace_with_placeholders($content)
    {
        ...
        $callback = function ($matches) {
            ...
            if ($block_or_hidden) {
                $placeholder_tag_name = EmailArtsHTMLFormatter::placeholder_block;
            } else {
                $placeholder_tag_name = EmailArtsHTMLFormatter::placeholder_inline;
            }
    ...
    private function scan_callback($matches, $replace = false)
    {
        ...
        if ($this->tag_type_supports($tag_type, 'name-attr')) {
            ...
            if (!wpea_is_name($scanned_tag['raw_name'])) {
        ...
        if (is_array($attr)) {
            ...
            if (WPEA_USE_PIPE) {
                $pipes = new WPEAPipes($scanned_tag['raw_values']);
        ...
        $scanned_tag = apply_filters('wpea_form_tag', $scanned_tag, $replace);

        $scanned_tag = new FormTag($scanned_tag);
    ...
    private function parse_atts($text)
    {
        ...
        if (preg_match($pattern, $text, $matches)) {
            ...
            if (!empty($matches[2])) {
                ...
                $atts['values'] = wpea_strip_quote_deep($matched_values[0]);
            }
        } else {
            $atts = $text;
        }
