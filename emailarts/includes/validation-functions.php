 ...
 */
function wpea_is_email( $text ) {
	$result = is_email( $text );
	return apply_filters( 'wpea_is_email', $result, $text );
}
 ...
 */
function wpea_is_tel( $text ) {
	$text = preg_replace( '%[()/.*#\s-]+%', '', $text );
	$result = preg_match( '/^[+]?[0-9]+$/', $text );
	return apply_filters( 'wpea_is_tel', $result, $text );
}
...
function wpea_is_number( $text ) {
	...
	return apply_filters( 'wpea_is_number', $result, $text );
}
...
function wpea_is_date( $text ) {
	...
	return apply_filters( 'wpea_is_date', $result, $text );
}
...
function wpea_is_time( $text ) {
	...
	return apply_filters( 'wpea_is_time', $result, $text );
}
 ...
 */
function wpea_is_mailbox_list( $mailbox_list ) {
 ...
 *              false if not.
 */
function wpea_is_email_in_domain( $email, $domain ) {
	$email_list = wpea_is_mailbox_list( $email );
 ...
 */
function wpea_is_email_in_site_domain( $email ) {
	...
	foreach ( $homes as $home ) {
		...
		if ( wpea_is_email_in_domain( $email, $sitename ) ) {
			return true;
		}
	}
 ...
 *              false otherwise.
 */
function wpea_is_file_path_in_content_dir( $path ) {
