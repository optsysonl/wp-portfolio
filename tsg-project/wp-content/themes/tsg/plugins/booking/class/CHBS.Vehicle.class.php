...
class CHBSVehicle
{
	...
	function setPostMetaDefault(&$meta)
	{
        ...
        CHBSHelper::setDefault($meta,'visibility_type',1);
    ...
    function getDictionary($attr=array(),$sortingType=1)
    {
		...
		while($query->have_posts())
		{
            ...
            $categories = get_the_terms($post->ID, $this->getCPTCategoryName());
			$dictionary[$post->ID]['post']=$post;
			$dictionary[$post->ID]['type_category'] = (!empty($categories)) ?$categories[0] : [];
    ...
    function calculatePrice($data,$calculateHiddenFee=true,$roundVehiclePrice=false,$bidData=array())
    {
        ...
        if(!is_null(CHBSCurrency::getFormCurrency())){
        ...
        if((int)$priceBase['price_type']===2){
            ...
            $priceSumGrossValue=number_format(
                $priceSumNetValue*(1+$TaxRate->getTaxRateValue($priceBase['price_fixed_tax_rate_id'],$taxRate)/100),
                2,
                '.',
                ''
            );
        ...
        else
        {
            if(in_array($data['service_type_id'],array(1,3)))
            {
                ...
                //TODO if we need calculate price by every passenger option - 'calculate_price_by_passenger_quantity'
                $quantity = 1;
                if((int) $data['booking_form']['meta']['calculate_price_by_passenger_quantity'] !== 0) {
                    if(
                        (CHBSBookingHelper::isPassengerEnable($data['booking_form']['meta'],$data['service_type_id'],'adult')) ||
                        (CHBSBookingHelper::isPassengerEnable($data['booking_form']['meta'],$data['service_type_id'],'children'))
                    ) {
                        $quantity = 0;
                        if(CHBSBookingHelper::isPassengerEnable($data['booking_form']['meta'],$data['service_type_id'],'adult')){
                            $quantity = $quantity + (int)$data['passenger_adult'];
                        }
                        if(CHBSBookingHelper::isPassengerEnable($data['booking_form']['meta'],$data['service_type_id'],'children')){
                            $quantity = $quantity + (int)$data['passenger_children'];
                        }
                        $quantity = ($quantity == 0) ? 1 : $quantity;
                    }
                }
                //end calculation
                $priceSumNetValue1 = $priceBase['price_distance_value'] * $distance * $quantity;

                $priceSumGrossValue=number_format(
                    $priceSumNetValue1*(1+$TaxRate->getTaxRateValue($priceBase['price_distance_tax_rate_id'],$taxRate)/100),
                    2,
                    '.',
                    ''
                );
                ...
                {
                    $priceSumNetValue2=$priceBase['price_hour_value']*$duration * $quantity;
                    $priceSumGrossValue+=number_format(
                        $priceSumNetValue2*(1+$TaxRate->getTaxRateValue($priceBase['price_hour_tax_rate_id'],$taxRate)/100),
                        2,
                        '.',
                        ''
                    );
...
//                {
////                    if((int) $data['passenger_adult'] !== 0){
////                        $priceSumNetValue = $priceSumNetValue * (int) $data['passenger_adult'] * $quantity;
////                    }
////                    if((int) $data['passenger_children'] !== 0){
////                        $priceSumNetValue = $priceSumNetValue * (int) $data['passenger_children'] * $quantity;
////                    }
//                    $price_distance = ($data['transfer_type_id'] == 2) ? 'price_distance_return_value' : 'price_distance_return_new_ride_value';
//                    $price_distance_tax_rate_id = ($data['transfer_type_id'] == 2) ? 'price_distance_return_tax_rate_id' : 'price_distance_return_new_ride_tax_rate_id';
//                    $price_hour_value = ($data['transfer_type_id'] == 2) ? 'price_hour_return_value' : 'price_hour_return_new_ride_value';
//                    $price_hour_tax_rate_id = ($data['transfer_type_id'] == 2) ? 'price_hour_return_tax_rate_id' : 'price_hour_return_new_ride_tax_rate_id';
//
//                    $priceSumNetValue1=$priceBase[$price_distance] * $distance * $quantity;
//                    $priceSumGrossValue+=number_format(
//                        $priceSumNetValue1*(1+$TaxRate->getTaxRateValue($priceBase[$price_distance_tax_rate_id],$taxRate)/100),
//                        2,
//                        '.',
...
//		}
        ...
        $price['price']['other']['net']['price_passenger_adult']['value']=$price['price']['base']['price_passenger_adult_value'];
        ...
        $price['price']['other']['net']['price_passenger_children']['value']=$price['price']['base']['price_passenger_children_value'];
