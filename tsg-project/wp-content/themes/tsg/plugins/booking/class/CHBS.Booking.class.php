...
//		add_filter('manage_edit-'.self::getCPTName().'_sortable_columns',array($this,'manageEditSortableColumns'));
    ...
    function change_formatted_meta_data( $meta_data, $item  ){
        ...
        foreach ( $meta_data as $id => $meta_array ) {
            if (
                'transfer-booking-id' === $meta_array->key ||
                'vehicle' === $meta_array->key ||
                'location-dropoff' === $meta_array->key ||
                'location-pickup' === $meta_array->key ||
                'passengers' === $meta_array->key ||
                'chauffeur-booking-system' === $meta_array->key
            ) {
                continue;
            }
        ...
        foreach( $values as $key => $value ) {
            switch ($key) {
                ...
                case 'date':
                    ...
                    $item->update_meta_data( __("Pickup Date", "chauffeur-booking-system"), $date);
                    break;
                case 'time':
                    $item->update_meta_data(__("Pickup Time", "chauffeur-booking-system"), $value);
                    break;
                case 'distance':
                    $item->update_meta_data(__("Distance", "chauffeur-booking-system"), $Length->convertUnit($value,1,2).' mi' );
                    break;
                case 'vehicle_name':
                    $item->update_meta_data(__("Vehicle Type", "chauffeur-booking-system"), $value);
                    break;
                case 'pickup_location_data':
                    $item->update_meta_data(__("Pickup Location", "chauffeur-booking-system"), $value);
                    break;
                case 'dropoff_location_data':
                    $item->update_meta_data(__("Dropoff Location", "chauffeur-booking-system"), $value);
                    break;
                case 'passenger_adult':
                    $item->update_meta_data(__("Adult Passengers", "chauffeur-booking-system"), $value);
                    break;
                case 'passenger_children':
                    if ($value !== '0') {
                        $item->update_meta_data(__("Children Passengers", "chauffeur-booking-system"), $value);
                    };
                    break;
                case 'airline':
                    $item->update_meta_data(__("Airline", "chauffeur-booking-system"), $value);
                    break;
                case 'flight':
                    $item->update_meta_data(__("Flight", "chauffeur-booking-system"), $value);
                    break;
        ...
        if( ! empty( $cart_item ) ){
            foreach( $cart_item as $key => $value ) {
                switch($key){
                    ...
                    case 'distance':
                        $cart_data[] = array(
                            'name'    => __( "Distance", "chauffeur-booking-system"),
                            'display' => $Length->convertUnit($value,1,2).' mi'
                        );
                        break;
                    case 'vehicle_name':
                        $cart_data[] = array(
                            'name'  => __("Vehicle Type", "chauffeur-booking-system"),
                            'display' => $value
                        );
                        break;
                    case 'pickup_location_data':
                    ...
                    case 'airline':
                        $cart_data[] = array(
                            'name' => __('Airline', "chauffeur-booking-system"),
                            'display' => $value,
                        );
                        break;
                    case 'flight':
                        $cart_data[] = array(
                            'name' => __('Flight', "chauffeur-booking-system"),
                            'display' => $value,
                        );
                        break;
                    case 'contact_information':
...
//        CHBSPostMeta::updatePostMeta($bookingId,'woocommerce_enable',$WooCommerce->isEnable($bookingForm['meta']));
        ...
        $vehiclePrice=$Vehicle->calculatePrice($argument,true,false);
        ...
        (
            ...
            //store vehicle price
            'arrival_price_type'                                                =>  $vehiclePrice['price']['base']['price_type'],
            'arrival_price_fixed_value'                                         =>  $vehiclePrice['price']['base']['price_fixed_value'],
            'arrival_price_fixed_tax_rate_value'                                =>  $TaxRate->getTaxRateValue($vehiclePrice['price']['base']['price_fixed_tax_rate_id'],$taxRateDictionary),
            'arrival_price_fixed_return_value'                                  =>  $vehiclePrice['price']['base']['price_fixed_return_value'],
            'arrival_price_fixed_return_tax_rate_value'                         =>  $TaxRate->getTaxRateValue($vehiclePrice['price']['base']['price_fixed_return_tax_rate_id'],$taxRateDictionary),
            'arrival_price_fixed_return_new_ride_value'                         =>  $vehiclePrice['price']['base']['price_fixed_return_new_ride_value'],
            'arrival_price_fixed_return_new_ride_tax_rate_value'                =>  $TaxRate->getTaxRateValue($vehiclePrice['price']['base']['price_fixed_return_new_ride_tax_rate_id'],$taxRateDictionary),
            'arrival_price_initial_value'                                       =>  $vehiclePrice['price']['base']['price_initial_value'],
            'arrival_price_initial_tax_rate_value'                              =>  $TaxRate->getTaxRateValue($vehiclePrice['price']['base']['price_initial_tax_rate_id'],$taxRateDictionary),
            'arrival_price_delivery_value'                                      =>  $vehiclePrice['price']['base']['price_delivery_value'],
            'arrival_price_delivery_tax_rate_value'                             =>  $TaxRate->getTaxRateValue($vehiclePrice['price']['base']['price_delivery_tax_rate_id'],$taxRateDictionary),
            'arrival_price_delivery_return_value'                               =>  $vehiclePrice['price']['base']['price_delivery_return_value'],
            'arrival_price_delivery_return_tax_rate_value'                      =>  $TaxRate->getTaxRateValue($vehiclePrice['price']['base']['price_delivery_return_tax_rate_id'],$taxRateDictionary),
            'arrival_price_distance_value'                                      =>  $vehiclePrice['price']['base']['price_distance_value'],
            'arrival_price_distance_tax_rate_value'                             =>  $TaxRate->getTaxRateValue($vehiclePrice['price']['base']['price_distance_tax_rate_id'],$taxRateDictionary),
            'arrival_price_distance_return_value'                               =>  $vehiclePrice['price']['base']['price_distance_return_value'],
            'arrival_price_distance_return_tax_rate_value'                      =>  $TaxRate->getTaxRateValue($vehiclePrice['price']['base']['price_distance_return_tax_rate_id'],$taxRateDictionary),
            'arrival_price_distance_return_new_ride_value'                      =>  $vehiclePrice['price']['base']['price_distance_return_new_ride_value'],
            'arrival_price_distance_return_new_ride_tax_rate_value'             =>  $TaxRate->getTaxRateValue($vehiclePrice['price']['base']['price_distance_return_new_ride_tax_rate_id'],$taxRateDictionary),
            'arrival_price_hour_value'                                          =>  $vehiclePrice['price']['base']['price_hour_value'],
            'arrival_price_hour_tax_rate_value'                                 =>  $TaxRate->getTaxRateValue($vehiclePrice['price']['base']['price_hour_tax_rate_id'],$taxRateDictionary),
            'arrival_price_hour_return_value'                                   =>  $vehiclePrice['price']['base']['price_hour_return_value'],
            'arrival_price_hour_return_tax_rate_value'                          =>  $TaxRate->getTaxRateValue($vehiclePrice['price']['base']['price_hour_return_tax_rate_id'],$taxRateDictionary),
            'arrival_price_hour_return_new_ride_value'                          =>  $vehiclePrice['price']['base']['price_hour_return_new_ride_value'],
            'arrival_price_hour_return_new_ride_tax_rate_value'                 =>  $TaxRate->getTaxRateValue($vehiclePrice['price']['base']['price_hour_return_new_ride_tax_rate_id'],$taxRateDictionary),
            'arrival_price_extra_time_value'                                    =>  $vehiclePrice['price']['base']['price_extra_time_value'],
            'arrival_price_extra_time_tax_rate_value'                           =>  $TaxRate->getTaxRateValue($vehiclePrice['price']['base']['price_extra_time_tax_rate_id'],$taxRateDictionary),
            'arrival_price_passenger_adult_value'                               =>  $vehiclePrice['price']['base']['price_passenger_adult_value'],
            'arrival_price_passenger_adult_tax_rate_value'                      =>  $TaxRate->getTaxRateValue($vehiclePrice['price']['base']['price_passenger_adult_tax_rate_id'],$taxRateDictionary),
            'arrival_price_passenger_children_value'                            =>  $vehiclePrice['price']['base']['price_passenger_children_value'],
            'arrival_price_passenger_children_tax_rate_value'                   =>  $TaxRate->getTaxRateValue($vehiclePrice['price']['base']['price_passenger_children_tax_rate_id'],$taxRateDictionary),
            'arrival_price_round_value'											=>	$vehiclePrice['price']['base']['round_value'],

            'arrival_price_sum_net_value'                                       => $vehiclePrice['price']['sum']['net']['value'],
            'arrival_price_sum_gross_value'                                     => $vehiclePrice['price']['sum']['gross']['value']
        );
        ...
        /***/
        /* vehicle return */
        $vehicle_return=$bookingForm['dictionary']['vehicle'][$data['vehicle_id_return']];

        // *rule
        $argument_return=array
        (
            'booking_form_id'                                                   =>  $data['booking_form_id'],
            'service_type_id'                                                   =>  $data['service_type_id'],
            'transfer_type_id'                                                  =>  $data['transfer_type_service_type_'.$data['service_type_id']],
            'pickup_location_coordinate'                                        =>  $data['dropoff_location_coordinate_service_type_'.$data['service_type_id']],
            'dropoff_location_coordinate'                                       =>  $data['pickup_location_coordinate_service_type_'.$data['service_type_id']],
            'fixed_location_pickup'                                             =>  $data['fixed_location_dropoff_service_type_'.$data['service_type_id']],
            'fixed_location_dropoff'                                            =>  $data['fixed_location_pickup_service_type_'.$data['service_type_id']],
            'route_id'                                                          =>  $data['route_service_type_3'],
            'vehicle_id'                                                        =>  $data['vehicle_id_return'],
            'pickup_date'                                                       =>  $data['pickup_date_service_type_'.$data['service_type_id']],
            'pickup_time'                                                       =>  $data['pickup_time_service_type_'.$data['service_type_id']],
            'base_location_distance'                                            =>  CHBSBookingHelper::getBaseLocationDistance($data['vehicle_id_return']),
            'base_location_return_distance'                                     =>  CHBSBookingHelper::getBaseLocationDistance($data['vehicle_id_return'],true),
            'distance'                                                          =>  $data['distance_map'],
            'distance_sum'                                                      =>  $data['distance_sum'],
            'duration'                                                          =>  in_array($data['service_type_id'],array(1,3)) ? 0 : $data['duration_service_type_2']*60,
            'duration_map'                                                      =>  $data['duration_map'],
        ...
        $vehicleReturnPrice=$Vehicle->calculatePrice($argument_return,true,false);

        CHBSPostMeta::updatePostMeta($bookingId,'vehicle_id_return',$WPML->translateID($data['vehicle_id_return']));
        CHBSPostMeta::updatePostMeta($bookingId,'vehicle_name_return',$vehicle_return['post']->post_title);

        $vehiclePriceBookingReturn=array
        (
            'price_type_return'                                                 =>  $vehicleReturnPrice['price']['base']['price_type'],
            'price_fixed_return_value'                                          =>  $vehicleReturnPrice['price']['base']['price_fixed_return_value'],
            'price_fixed_return_tax_rate_value'                                 =>  $TaxRate->getTaxRateValue($vehicleReturnPrice['price']['base']['price_fixed_return_tax_rate_id'],$taxRateDictionary),
            'price_fixed_return_new_ride_value'                                 =>  $vehicleReturnPrice['price']['base']['price_fixed_return_new_ride_value'],
            'price_fixed_return_new_ride_tax_rate_value'                        =>  $TaxRate->getTaxRateValue($vehicleReturnPrice['price']['base']['price_fixed_return_new_ride_tax_rate_id'],$taxRateDictionary),
            'price_delivery_return_value'                                       =>  $vehicleReturnPrice['price']['base']['price_delivery_return_value'],
            'price_delivery_return_tax_rate_value'                              =>  $TaxRate->getTaxRateValue($vehicleReturnPrice['price']['base']['price_delivery_return_tax_rate_id'],$taxRateDictionary),
            'price_distance_return_value'                                       =>  $vehicleReturnPrice['price']['base']['price_distance_return_value'],
            'price_distance_return_tax_rate_value'                              =>  $TaxRate->getTaxRateValue($vehicleReturnPrice['price']['base']['price_distance_return_tax_rate_id'],$taxRateDictionary),
            'price_distance_return_new_ride_value'                              =>  $vehicleReturnPrice['price']['base']['price_distance_return_new_ride_value'],
            'price_distance_return_new_ride_tax_rate_value'                     =>  $TaxRate->getTaxRateValue($vehicleReturnPrice['price']['base']['price_distance_return_new_ride_tax_rate_id'],$taxRateDictionary),
            'price_hour_return_value'                                           =>  $vehicleReturnPrice['price']['base']['price_hour_return_value'],
            'price_hour_return_tax_rate_value'                                  =>  $TaxRate->getTaxRateValue($vehicleReturnPrice['price']['base']['price_hour_return_tax_rate_id'],$taxRateDictionary),
            'price_hour_return_new_ride_value'                                  =>  $vehicleReturnPrice['price']['base']['price_hour_return_new_ride_value'],
            'price_hour_return_new_ride_tax_rate_value'                         =>  $TaxRate->getTaxRateValue($vehicleReturnPrice['price']['base']['price_hour_return_new_ride_tax_rate_id'],$taxRateDictionary),
            'price_round_value'													=>	0,
//          store return vehicle price
            'return_price_type'                                                 =>  $vehicleReturnPrice['price']['base']['price_type'],
            'return_price_fixed_value'                                          =>  $vehicleReturnPrice['price']['base']['price_fixed_value'],
            'return_price_fixed_tax_rate_value'                                 =>  $TaxRate->getTaxRateValue($vehicleReturnPrice['price']['base']['price_fixed_tax_rate_id'],$taxRateDictionary),
            'return_price_fixed_return_value'                                   =>  $vehicleReturnPrice['price']['base']['price_fixed_return_value'],
            'return_price_fixed_return_tax_rate_value'                          =>  $TaxRate->getTaxRateValue($vehicleReturnPrice['price']['base']['price_fixed_return_tax_rate_id'],$taxRateDictionary),
            'return_price_fixed_return_new_ride_value'                          =>  $vehicleReturnPrice['price']['base']['price_fixed_return_new_ride_value'],
            'return_price_fixed_return_new_ride_tax_rate_value'                 =>  $TaxRate->getTaxRateValue($vehicleReturnPrice['price']['base']['price_fixed_return_new_ride_tax_rate_id'],$taxRateDictionary),
            'return_price_initial_value'                                        =>  $vehicleReturnPrice['price']['base']['price_initial_value'],
            'return_price_initial_tax_rate_value'                               =>  $TaxRate->getTaxRateValue($vehicleReturnPrice['price']['base']['price_initial_tax_rate_id'],$taxRateDictionary),
            'return_price_delivery_value'                                       =>  $vehicleReturnPrice['price']['base']['price_delivery_value'],
            'return_price_delivery_tax_rate_value'                              =>  $TaxRate->getTaxRateValue($vehicleReturnPrice['price']['base']['price_delivery_tax_rate_id'],$taxRateDictionary),
            'return_price_delivery_return_value'                                =>  $vehicleReturnPrice['price']['base']['price_delivery_return_value'],
            'return_price_delivery_return_tax_rate_value'                       =>  $TaxRate->getTaxRateValue($vehicleReturnPrice['price']['base']['price_delivery_return_tax_rate_id'],$taxRateDictionary),
            'return_price_distance_value'                                       =>  $vehicleReturnPrice['price']['base']['price_distance_value'],
            'return_price_distance_tax_rate_value'                              =>  $TaxRate->getTaxRateValue($vehicleReturnPrice['price']['base']['price_distance_tax_rate_id'],$taxRateDictionary),
            'return_price_distance_return_value'                                =>  $vehicleReturnPrice['price']['base']['price_distance_return_value'],
            'return_price_distance_return_tax_rate_value'                       =>  $TaxRate->getTaxRateValue($vehicleReturnPrice['price']['base']['price_distance_return_tax_rate_id'],$taxRateDictionary),
            'return_price_distance_return_new_ride_value'                       =>  $vehicleReturnPrice['price']['base']['price_distance_return_new_ride_value'],
            'return_price_distance_return_new_ride_tax_rate_value'              =>  $TaxRate->getTaxRateValue($vehicleReturnPrice['price']['base']['price_distance_return_new_ride_tax_rate_id'],$taxRateDictionary),
            'return_price_hour_value'                                           =>  $vehicleReturnPrice['price']['base']['price_hour_value'],
            'return_price_hour_tax_rate_value'                                  =>  $TaxRate->getTaxRateValue($vehicleReturnPrice['price']['base']['price_hour_tax_rate_id'],$taxRateDictionary),
            'return_price_hour_return_value'                                    =>  $vehicleReturnPrice['price']['base']['price_hour_return_value'],
            'return_price_hour_return_tax_rate_value'                           =>  $TaxRate->getTaxRateValue($vehicleReturnPrice['price']['base']['price_hour_return_tax_rate_id'],$taxRateDictionary),
            'return_price_hour_return_new_ride_value'                           =>  $vehicleReturnPrice['price']['base']['price_hour_return_new_ride_value'],
            'return_price_hour_return_new_ride_tax_rate_value'                  =>  $TaxRate->getTaxRateValue($vehicleReturnPrice['price']['base']['price_hour_return_new_ride_tax_rate_id'],$taxRateDictionary),
            'return_price_extra_time_value'                                     =>  $vehicleReturnPrice['price']['base']['price_extra_time_value'],
            'return_price_extra_time_tax_rate_value'                            =>  $TaxRate->getTaxRateValue($vehicleReturnPrice['price']['base']['price_extra_time_tax_rate_id'],$taxRateDictionary),
            'return_price_passenger_adult_value'                                =>  $vehicleReturnPrice['price']['base']['price_passenger_adult_value'],
            'return_price_passenger_adult_tax_rate_value'                       =>  $TaxRate->getTaxRateValue($vehicleReturnPrice['price']['base']['price_passenger_adult_tax_rate_id'],$taxRateDictionary),
            'return_price_passenger_children_value'                             =>  $vehicleReturnPrice['price']['base']['price_passenger_children_value'],
            'return_price_passenger_children_tax_rate_value'                    =>  $TaxRate->getTaxRateValue($vehicleReturnPrice['price']['base']['price_passenger_children_tax_rate_id'],$taxRateDictionary),
            'return_price_round_value'											=>	$vehicleReturnPrice['price']['base']['round_value'],

            'return_price_sum_net_value'                                        => $vehicleReturnPrice['price']['sum']['net']['value'],
            'return_price_sum_gross_value'                                      => $vehicleReturnPrice['price']['sum']['gross']['value']
        );

        foreach($vehiclePriceBookingReturn as $index=>$value) {
            CHBSPostMeta::updatePostMeta($bookingId, $index, $value);
        }
...
//        }
        ...
        if($data['transfer_type_service_type_'.$data['service_type_id']] === '3') {
            ...
            CHBSPostMeta::updatePostMeta($bookingId, 'departure_flight', $data['form_element_field_departure_flight']);
        }
...
//        $taxRateDictionary=$TaxRate->getDictionary();
        ...
        $component=array('initial','initial_return','initial_total','delivery','delivery_return','vehicle','vehicle_return','extra_time','booking_extra','booking_extra_return','total','pay');
...
//                $price['vehicle']['sum']['net']['value']+=$price['initial']['sum']['net']['value']+$price['delivery']['sum']['net']['value']+$price['delivery_return']['sum']['net']['value']+$price['extra_time']['sum']['net']['value'];
                $price['vehicle']['sum']['net']['value']+=$price['initial']['sum']['net']['value']+$price['delivery']['sum']['net']['value']+$price['extra_time']['sum']['net']['value'];
...
//            }
        ...
        if($data['transfer_type_service_type_'.$data['service_type_id']] == 3 && array_key_exists($data['vehicle_id_return'],$data['booking_form']['dictionary']['vehicle']))
        {
            $serviceTypeId=$data['service_type_id'];
            ...
            $vehiclePriceReturn = null;
            if(is_null($vehiclePriceReturn))
            {
                $vehiclePriceReturn=$Vehicle->calculatePrice($argument,false);
            }
            ...
            if(array_key_exists('other',$vehiclePriceReturn['price'])) {
                $price['other'] = $vehiclePriceReturn['price']['other'];
            }

            $price['vehicle_return']['sum']['net']['value']=$vehiclePriceReturn['price']['sum']['net']['value'];
            $price['vehicle_return']['sum']['gross']['value']=$vehiclePriceReturn['price']['sum']['gross']['value'];

            $price['initial_return']['sum']['net']['value']=$vehiclePriceReturn['price']['base']['price_initial_value'];
            $price['initial_return']['sum']['gross']['value']=CHBSPrice::calculateGross($vehiclePriceReturn['price']['base']['price_initial_value'],$vehiclePriceReturn['price']['base']['price_initial_tax_rate_id']);

//            $price['delivery']['sum']['net']['value']=$vehiclePriceReturn['price']['base']['price_delivery_value']*$data['base_location_distance'];
//            $price['delivery']['sum']['gross']['value']=CHBSPrice::calculateGross($vehiclePriceReturn['price']['base']['price_delivery_value']*$data['base_location_distance'],$vehiclePriceReturn['price']['base']['price_delivery_tax_rate_id']);

            $price['delivery_return']['sum']['net']['value']=$vehiclePriceReturn['price']['base']['price_delivery_return_value']*$data['base_location_return_distance'];
            $price['delivery_return']['sum']['gross']['value']=CHBSPrice::calculateGross($vehiclePriceReturn['price']['base']['price_delivery_return_value']*$data['base_location_return_distance'],$vehiclePriceReturn['price']['base']['price_delivery_return_tax_rate_id']);
...
//            if(in_array($serviceTypeId,array(1,3)))
//            {
//                $priceExtraTime=$vehiclePriceReturn['price']['base']['price_extra_time_value'];
//
//                $time=$data['extra_time_service_type_'.$data['service_type_id']];
//
//                if((int)$data['booking_form']['meta']['extra_time_unit']===1) $time/=60;
//
//                $price['extra_time']['sum']['net']['value']=$priceExtraTime*$time;
//                $price['extra_time']['sum']['gross']['value']=CHBSPrice::calculateGross($price['extra_time']['sum']['net']['value'],$vehiclePriceReturn['price']['base']['price_extra_time_tax_rate_id']);
//            }


            if((int)$vehiclePriceReturn['price']['base']['price_type']===2)
            {
                $price['initial_return']['sum']['net']['value']=0.00;
                $price['initial_return']['sum']['gross']['value']=0.00;
            ...
            if($hideFee)
            {
                $price['vehicle_return']['sum']['net']['value']+=$price['initial']['sum']['net']['value']+$price['delivery']['sum']['net']['value']+$price['delivery_return']['sum']['net']['value']+$price['extra_time']['sum']['net']['value'];
                $price['vehicle_return']['sum']['gross']['value']+=$price['initial']['sum']['gross']['value']+$price['delivery']['sum']['gross']['value']+$price['delivery_return']['sum']['gross']['value']+$price['extra_time']['sum']['gross']['value'];
            }

            /***/

            $price['initial_return']['sum']['net']['format']=CHBSPrice::format($price['initial_return']['sum']['net']['value'],CHBSCurrency::getFormCurrency());
            $price['initial_return']['sum']['gross']['format']=CHBSPrice::format($price['initial_return']['sum']['gross']['value'],CHBSCurrency::getFormCurrency());
...
//            }
            ...
            $price['vehicle_return']['sum']['net']['format']=CHBSPrice::format($price['vehicle_return']['sum']['net']['value'],CHBSCurrency::getFormCurrency());
            $price['vehicle_return']['sum']['gross']['format']=CHBSPrice::format($price['vehicle_return']['sum']['gross']['value'],CHBSCurrency::getFormCurrency());

            $price['vehicle_return']['sum']['net']['formatHtml']=$Vehicle->getPriceFormatHtml($price['vehicle_return']['sum']['net']['value']);
            $price['vehicle_return']['sum']['gross']['formatHtml']=$Vehicle->getPriceFormatHtml($price['vehicle_return']['sum']['gross']['value']);

            /***/

        }

        //combined arrival & return vehicle prices

        $vehicle_sum = (isset($price['vehicle_return']['sum']['net']['value'])) ? (float)$price['vehicle']['sum']['net']['value']+(float)$price['vehicle_return']['sum']['net']['value'] : $price['vehicle']['sum']['net']['value'];
        $price['vehicle_grouped']['sum']['net']['value'] = $vehicle_sum;
        $price['vehicle_grouped']['sum']['net']['format'] = CHBSPrice::format($price['vehicle_grouped']['sum']['net']['value'], CHBSCurrency::getFormCurrency());
        $price['vehicle_grouped']['sum']['net']['formatHtml'] = $Vehicle->getPriceFormatHtml($price['vehicle_grouped']['sum']['net']['value']);
        $price['vehicle_grouped']['sum']['gross']['value'] = $vehicle_sum;
        $price['vehicle_grouped']['sum']['gross']['format'] = CHBSPrice::format($price['vehicle_grouped']['sum']['gross']['value'], CHBSCurrency::getFormCurrency());
        $price['vehicle_grouped']['sum']['gross']['formatHtml'] = $Vehicle->getPriceFormatHtml($price['vehicle_grouped']['sum']['gross']['value']);

        $price['initial_total']['sum']['net']['value'] = $price['initial']['sum']['net']['value'] + $price['initial_return']['sum']['net']['value'];
        $price['initial_total']['sum']['net']['format'] = CHBSPrice::format($price['initial_total']['sum']['net']['value'], CHBSCurrency::getFormCurrency());
        $price['initial_total']['sum']['gross']['value'] = $price['initial_total']['sum']['net']['value'];
        $price['initial_total']['sum']['gross']['format'] = CHBSPrice::format($price['initial_total']['sum']['gross']['value'], CHBSCurrency::getFormCurrency());
...
//                $price['booking_extra']['sum']['net']['value']+=$value['quantity']*(float)$value['price'];
//                $price['booking_extra']['sum']['gross']['value']+=CHBSPrice::calculateGross($value['quantity']*$value['price'],0,$value['tax_rate_value']);
//            }
//            if(isset($value['quantity_return'])){
//                $price['booking_extra_return']['sum']['net']['value']+=$value['quantity_return']*(float)$value['price'];
//                $price['booking_extra_return']['sum']['gross']['value']+=CHBSPrice::calculateGross($value['quantity_return']*$value['price'],0,$value['tax_rate_value']);
//            }
////            $quantity = (isset($value['quantity_return']) && !empty($value['quantity_return'])) ? $value['quantity'] + $value['quantity_return'] : $value['quantity'];
////            $price['booking_extra']['sum']['net']['value']+=$quantity*(int)$value['price'];
////            $price['booking_extra']['sum']['gross']['value']+=CHBSPrice::calculateGross($quantity*$value['price'],0,$value['tax_rate_value']);
//        }
//
...
//        $price['booking_extra_return']['sum']['net']['format']=CHBSPrice::format($price['booking_extra_return']['sum']['net']['value'],CHBSCurrency::getFormCurrency());
//        $price['booking_extra_return']['sum']['gross']['format']=CHBSPrice::format($price['booking_extra_return']['sum']['gross']['value'],CHBSCurrency::getFormCurrency());
//
//        $price['booking_extra_total']['sum']['net']['value'] = $price['booking_extra_return']['sum']['net']['value'] + $price['booking_extra']['sum']['net']['value'];
//        $price['booking_extra_total']['sum']['net']['format'] = CHBSPrice::format($price['booking_extra_total']['sum']['net']['value'],CHBSCurrency::getFormCurrency());
//        $price['booking_extra_total']['sum']['gross']['value'] = $price['booking_extra_return']['sum']['gross']['value'] + $price['booking_extra']['sum']['gross']['value'];
//        $price['booking_extra_total']['sum']['gross']['format'] = CHBSPrice::format($price['booking_extra_total']['sum']['gross']['value'],CHBSCurrency::getFormCurrency());

        /***/

        if($hideFee)
        {
//            $price['total']['sum']['net']['value']=$price['vehicle']['sum']['net']['value']+$price['vehicle_return']['sum']['net']['value']+$price['booking_extra']['sum']['net']['value'];
            $price['total']['sum']['net']['value']=$price['vehicle']['sum']['net']['value']+$price['vehicle_return']['sum']['net']['value']+$price['booking_extra']['sum']['net']['value']+$price['booking_extra_return']['sum']['net']['value'];
        }
        else
        {
//            $price['total']['sum']['net']['value']=$price['initial']['sum']['net']['value']+$price['delivery']['sum']['net']['value']+$price['delivery_return']['sum']['net']['value']+$price['extra_time']['sum']['net']['value']+$price['vehicle']['sum']['net']['value']+$price['booking_extra']['sum']['net']['value'];
            $price['total']['sum']['net']['value']=$price['initial']['sum']['net']['value']+$price['initial_return']['sum']['net']['value']+$price['delivery']['sum']['net']['value']+$price['delivery_return']['sum']['net']['value']+$price['extra_time']['sum']['net']['value']+$price['vehicle']['sum']['net']['value']+$price['vehicle_return']['sum']['net']['value']+$price['booking_extra']['sum']['net']['value']+$price['booking_extra_return']['sum']['net']['value'];
        }
        ...
        if($hideFee)
        {
//            $price['total']['sum']['gross']['value']=$price['vehicle']['sum']['gross']['value']+$price['vehicle_return']['sum']['gross']['value']+$price['booking_extra']['sum']['gross']['value'];
            $price['total']['sum']['gross']['value']=$price['vehicle']['sum']['gross']['value']+$price['vehicle_return']['sum']['gross']['value']+$price['booking_extra']['sum']['gross']['value']+$price['booking_extra_return']['sum']['gross']['value'];
        }
        else
        {
//            $price['total']['sum']['gross']['value']=$price['initial']['sum']['gross']['value']+$price['delivery']['sum']['gross']['value']+$price['delivery_return']['sum']['gross']['value']+$price['extra_time']['sum']['gross']['value']+$price['vehicle']['sum']['gross']['value']+$price['booking_extra']['sum']['gross']['value'];
            $price['total']['sum']['gross']['value']=$price['initial']['sum']['gross']['value']+$price['initial_return']['sum']['gross']['value']+$price['delivery']['sum']['gross']['value']+$price['delivery_return']['sum']['gross']['value']+$price['extra_time']['sum']['gross']['value']+$price['vehicle']['sum']['gross']['value']+$price['vehicle_return']['sum']['gross']['value']+$price['booking_extra']['sum']['gross']['value']+$price['booking_extra_return']['sum']['gross']['value'];
        }
    ...
    function createBilling($bookingId, $data=array())
    {
        ...
        $_product_addons = WC_Product_Addons_Helper::get_product_addons( $bookingMeta['booking_source_product'] );
        $product_addons = [];
        ...
        $image_id = get_post_thumbnail_id( $booking['meta']['vehicle_id'] );
        $arrivalInitialPrice = $bookingMeta['arrival_price_initial_value'];
        $arrivalPriceNet = $bookingMeta['arrival_price_sum_gross_value'] + $arrivalInitialPrice;
        $arrivalValueNet = $arrivalPriceNet;
        $arrivalTaxValue = 0;

        $chauffeur_service=array
        (
            'type'                                                  =>  'chauffeur_service',
            'name'                                                  =>  __('Airport Transfer','chauffeur-booking-system'),
            'vehicle_name'                                          => $bookingMeta['vehicle_name'],
            'unit'                                                  =>  $unit,
            'quantity'                                              =>  1,
            'duration'                                              =>  $bookingMeta['duration'],
            'distance'                                              =>  $bookingMeta['distance'],
            'price_net'                                             =>  $arrivalPriceNet,
            'value_net'                                             =>  $arrivalValueNet,
            'tax_value'                                             =>  $arrivalTaxValue,
            'value_gross'                                           =>  CHBSPrice::calculateGross($arrivalValueNet,0,$arrivalTaxValue),
            'date'                                                  => $bookingMeta['pickup_date'],
            'time'                                                  => $bookingMeta['pickup_time'],
            'pickup_location_data'                                  => $bookingMeta['coordinate'][0]['formatted_address'].' ('.$bookingMeta['coordinate'][0]['address'].')',
            'pickup_location_short'                                 => $bookingMeta['coordinate'][0]['formatted_address'],
            'dropoff_location_data'                                 => $bookingMeta['coordinate'][1]['formatted_address'].' ('.$bookingMeta['coordinate'][1]['address'].')',
            'dropoff_location_short'                                => $bookingMeta['coordinate'][1]['formatted_address'],
            'airline'                                               => $data['form_element_field_arrival_airline'],
            'flight'                                                => $data['form_element_field_arrival_flight'],
            'image_id'                                              => $image_id,
            'booking_extra'                                         => array()
        );

        $selected_extra = explode(',',$data['booking_extra_id']);
        if(!empty($selected_extra)) {
            foreach ($selected_extra as $extra){
                $extra_name = str_replace('chbs_', '', $extra);
                if(isset($data[$extra_name]) && $data[$extra_name] !== 0){
                    $extra_key = str_replace('_arrival', '', $extra_name);
                    $chauffeur_service['booking_extra'][$extra_key] = $data[$extra_name];
                }
            }
        }
        ...
        if(in_array($booking['meta']['transfer_type_id'],array(2,3))){
            $image_id_return = get_post_thumbnail_id( $booking['meta']['vehicle_id_return'] );
            $returnInitialPrice = $bookingMeta['return_price_initial_value'];
            $returnPriceNet = $bookingMeta['return_price_sum_gross_value']+$returnInitialPrice;
            $returnValueNet = $returnPriceNet;
            $returnTaxValue = 0;
            $chauffeur_service_return=array
            (
                'type'                                          =>  'chauffeur_service_return',
                'name'                                          =>  __('Airport Transfer (return)','chauffeur-booking-system'),
                'vehicle_name'                                  => $bookingMeta['vehicle_name_return'],
                'unit'                                          =>  $unit,
                'quantity'                                      =>  1,
                'duration'                                      =>  $bookingMeta['duration'],
                'distance'                                      =>  $bookingMeta['distance'],
                'price_net'                                     =>  $returnPriceNet,
                'value_net'                                     =>  $returnValueNet,
                'tax_value'                                     =>  $returnTaxValue,
                'value_gross'                                   =>  CHBSPrice::calculateGross($returnValueNet,0,$returnTaxValue),
                'date'                                          => $bookingMeta['return_date'],
                'time'                                          => $bookingMeta['return_time'],
                'pickup_location_data'                          => $bookingMeta['coordinate'][1]['formatted_address'].' ('.$bookingMeta['coordinate'][1]['address'].')',
                'pickup_location_short'                         => $bookingMeta['coordinate'][1]['formatted_address'],
                'dropoff_location_data'                         => $bookingMeta['coordinate'][0]['formatted_address'].' ('.$bookingMeta['coordinate'][0]['address'].')',
                'dropoff_location_short'                        => $bookingMeta['coordinate'][0]['formatted_address'],
                'airline'                                       => $data['form_element_field_departure_airline'],
                'flight'                                        => $data['form_element_field_departure_flight'],
                'image_id'                                      => $image_id_return,
                'booking_extra'                                 => array()
            );

            $selected_extra_return = explode(',',$data['booking_extra_return_id']);
            if(!empty($selected_extra_return)) {
                foreach ($selected_extra_return as $extra){
                    $extra_name = str_replace('chbs_', '', $extra);
                    if(isset($data[$extra_name]) && $data[$extra_name] !== 0){
                        $extra_key = str_replace('_return', '', $extra_name);
                        $chauffeur_service_return['booking_extra'][$extra_key] = $data[$extra_name];
                    }
                }
            }
    ...
    function createBillingOld($bookingId, $data=array())
    {
        ...
        // Airport Transfer (passengers): option enabled in booking form and price type has to be marked as "Variable"
        if(($booking['meta']['passenger_enable']==1) && ($booking['meta']['price_type']==1))
        {
            ...
            if($booking['meta']['passenger_children_number']>0)
            {
                ...
                (
                    ...
                    'duration'                                                  =>  ($isPassengerAdult ? 0: $duration),
                    'distance'                                                  =>  ($isPassengerAdult ? 0: $distance),
        ...
        else
        {
            ...
            if(($booking['meta']['calculation_method']==2) && ($booking['meta']['price_type']==1))
            {
                ...
                (
                    ...
                    'name'														=>  __('Airport Transfer (duration)','chauffeur-booking-system'),
                ...
                (
                    ...
                    'name'														=>  __('Airport Transfer (distance)','chauffeur-booking-system'),
                    'unit'														=>  $Length->getUnitName($booking['meta']['length_unit']),
                    'quantity'													=>  $distance,
                    'duration'													=>  0,
                    'distance'													=>  $distance,
            ...
            else
            {
                ...
                if($priceNet>0)
                {
                    ...
                    $extra = $booking['meta']['booking_extra'];
                    if(!is_array($extra)) {
                        ...
                        if($booking['meta']['price_type']==1)
                        {
                            ...
                            if($valueNetDuration>0)
                            {
                                ...
                                (
                                    ...
                                    'name'                                      =>  __('Airport Transfer (duration, return)','chauffeur-booking-system'),
                                    'unit'                                      =>  __('Hours','chauffeur-booking-system'),
                                    'quantity'                                  =>  $Date->formatMinuteToTime($duration),
                                    'duration'                                  =>  $duration,
                                    'distance'                                  =>  0,
                            ...
                            if($valueNetDistance>0)
                            {
                                ...
                                (
                                    ...
                                    'name'                                      =>  __('Airport Transfer (distance, return)','chauffeur-booking-system'),
                                    'unit'                                      =>  $Length->getUnitName($booking['meta']['length_unit']),
                                    'quantity'                                  =>  $distance,
                                    'duration'                                  =>  0,
                                    'distance'                                  =>  $distance,
